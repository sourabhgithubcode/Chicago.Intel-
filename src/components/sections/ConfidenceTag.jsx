...
export default function ConfidenceTag({ score, source }) {
  ...
  return (
    <span
      className={`glass-3 label-mono inline-flex max-w-full flex-wrap items-center gap-1.5 px-2.5 py-1 ${color}`}
        ...
        <>
          ...
          {source.url ? (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="text-t1 hover:text-t0 break-words underline-offset-2 hover:underline"
            >
              {source.label}
            </a>
          ) : (
            <span className="text-t1 break-words">{source.label}</span>
          )}
        </>
      )}
    </span>
